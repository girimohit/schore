  ...
  try {
    ...
    if (type === "class") {
      ...
      if (role === UserRole.STUDENT) {
        ...
        if (!activeEnrollment) {
          return ApiResponse.badRequest("Student has no active class enrollment");
        }
      ...
      if (!classId || !sectionId) {
        return ApiResponse.badRequest("Missing classId or sectionId parameters");
      }

      const data = await timetableService.getClassTimetable(schoolId, classId, sectionId);
      return ApiResponse.success(data, "Class timetable retrieved successfully");
    } 
    ...
    if (type === "faculty") {
      ...
      if (role === UserRole.FACULTY) {
        const facultyService = new FacultyService();
        const faculty = await facultyService.getFacultyById(schoolId, facultyId);
        if (faculty.userId !== userId) {
          return ApiResponse.forbidden("Faculty can only view their own teaching timetable");
        }
      } else if (role === UserRole.STUDENT) {
        return ApiResponse.forbidden("Students cannot access teacher teaching schedules");
      }

      const data = await timetableService.getFacultyTimetable(schoolId, facultyId);
      return ApiResponse.success(data, "Faculty timetable retrieved successfully");
    }

    return ApiResponse.badRequest("Invalid query type parameter. Must be 'class' or 'faculty'");
  } catch (error: any) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (role !== UserRole.SUPER_ADMIN && role !== UserRole.SCHOOL_ADMIN) {
      return ApiResponse.forbidden("Only administrators can manage the timetable");
    }
    ...
    return ApiResponse.success(data, "Timetable entry created successfully", 201);
  } catch (error: any) {
    return ApiResponse.badRequest(error.message || "Failed to create timetable entry");
  }
}
